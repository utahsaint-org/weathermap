...
app.get('/api2/utilization/:circuits', function(req, res) {
    queryInterfaces(function(err, circuit_list) {
	if (err) return res.json({error: err});

	var client = influx({"host":"localhost", "database": "circuits"});

	var circuits = {};
	circuit_list.forEach(function(circuit) {
	    circuits[circuit.name] = { "bandwidth": circuit.bandwidth, "poll_frequency": circuit.poll_frequency };
	});
	circuit_list = undefined;

	var req_circuits = req.params.circuits.split(',');
	console.log('req_circuits', req_circuits.length);
	var results = {	"timestamp": Date.now() };
	var counter = 0;
	req_circuits.forEach(function(name) {
	    counter++;
	    console.log(name, 'counter=', counter);
	    name = name.trim();

	    if (circuits[name]) {
		var circuit = circuits[name];
		var ctr = counter;
		results[name] = { "bandwidth": circuit.bandwidth, "poll_frequency": circuit.poll_frequency };

		client.query('SELECT bps FROM "' + name + '.A.' + circuit.poll_frequency + 's" LIMIT 1', function(err, data) {
		    if (!err && data && data[0] && data[0].points && data[0].points[0] && data[0].points[0][2]) {
			results[name].A = Math.floor(data[0].points[0][2]);
			results[name].timestamp_A = data[0].points[0][0] / 1000;
			console.log(name + ".A." + circuit.poll_frequency + "s");
		    }

		    client.query('SELECT bps FROM "' + name + '.Z.' + circuit.poll_frequency + 's" LIMIT 1', function(err, data) {
			if (!err && data && data[0] && data[0].points && data[0].points[0] && data[0].points[0][2]) {
			    results[name].Z = Math.floor(data[0].points[0][2]);
			    results[name].timestamp_Z = data[0].points[0][0] / 1000;
			    console.log(name + ".Z." + circuit.poll_frequency + "s");
			}

			if (ctr >= req_circuits.length) {
			    console.log('done', name, counter, results);
			    res.json(results);
			}
		    });
		});
